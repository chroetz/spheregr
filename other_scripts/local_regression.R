...
cat("geo", system.time({
  geo <- estimate_geodesic(x, y, x_new, max_speed=5)
})[3], "\n")
geo_a <- geo$estim_a
...
lines_mercator(trigeo_a, palette=palette_const(3), lwd=8)
lines_mercator(trigeo_a, palette=palette_const(1), lwd=4)
lines_mercator(trigeo_a, palette=palette, lwd=2)
